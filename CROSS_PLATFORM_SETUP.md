...
All three are logic-identical at v2.6.1: the same 12 health checks, the same heartbeat-based quorum counting, the same anti-flap cooldown + hysteresis, the same DigiDollar BIP9 pre-activation guard, the same headless/Qt-wallet auto-detect, the same Discord embeds, the same optional email alerts on the same triggers (added in v2.6.0), the same daily update-check footer (added in v2.6.0), and the same cosmetic terminal-alignment fix that gives every ⚠️/ℹ️ status line a second space so they align with ✅/🔴 lines regardless of the terminal's emoji-width handling (added in v2.6.1). If you've seen my alerts in #oracle-alerts, these produce the same ones. The only differences are the platform plumbing underneath.
