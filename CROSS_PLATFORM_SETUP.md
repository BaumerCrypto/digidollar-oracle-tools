...
All three config files expose the same knobs with the same defaults: alert thresholds (`MIN_PEERS=3`, `MIN_DISK_GB=5`, `MEM_THRESHOLD=90`, `MAX_CHAIN_BEHIND=10`), quorum bands (`QUORUM_GREEN=12`, `QUORUM_YELLOW=10` — red and critical come from the chain's own `oracle_consensus_required`, never hardcoded), and the anti-flap controls (`QUORUM_COOLDOWN=30` minutes, `QUORUM_HYSTERESIS=3`). Escalation alerts always fire immediately; only recovery alerts are throttled.

**v2.7.0 adds the disk-safety knobs, same names on every platform:** `DISK_USED_PCT_WARN=80` (yellow disk band — closes #33), `DEBUG_LOG_WARN_MB=1024` (Check 13 debug.log watchdog, names any enabled debug categories in the alert), `DEBUG_LOG_ROTATE="yes"` / `DEBUG_LOG_MAX_MB=2048` / `DEBUG_LOG_KEEP=1` (safe auto-rotation — **default ON**, copy-then-truncate, blue card on every rotation, red card instead of rotation when free space can't hold the safety copy), and `PRICE_CHECK_EVERY=1` (run the loudest RPC — `getoracleprice`, ~4,780 log lines per call with `debug=digidollar` on — every Nth pass). Full rationale and the measured growth numbers live in the [hardening guide's debug.log section](https://github.com/BaumerCrypto/digidollar-oracle-tools/blob/main/ORACLE_HARDENING_GUIDE.md#debuglog-growth-rotation-and-the-disappearing-disk).
