...
#          macOS note: NETWORK_TAG also reaches the watch mode banner,
#          a fifth label site the Linux script does not have.
...
# (v2.9.0, #41). Empty by default, so nothing changes for anyone who
...
case "$ACTION_FLAG" in
    ...
    --watch)
        ...
        while true; do
            clear 2>/dev/null
            echo "🔭 ${NETWORK_TAG:-Oracle} Monitor — watch mode (refreshes every ${WATCH_INTERVAL}s, Ctrl+C to exit)"
            ISSUES=0
            WARNINGS=0
            DETAILS=""
            send_summary
