...
# Version: 2.2-macos.1
#
# macOS port of my oracle-monitor.sh v2.2 (Linux). Same checks, same quorum
# state machine, same anti-flap logic — BSD/macOS-native commands. Written
# for the stock /bin/bash 3.2 that ships with every Mac (no Homebrew bash
# needed). The only dependency is jq.
#
...
# Process name of your node:
#   "digibyted"     headless daemon (recommended for oracles)
#   "DigiByte-Qt"   if you run the Qt wallet app instead
...
check_daemon() {
    if pgrep -x "$DAEMON_PROCESS" > /dev/null 2>&1; then
        if clear_alert "daemon_down"; then
            alert_green "✅ Node Recovered" "$DAEMON_PROCESS is running again."
        fi
        DETAILS+="✅ $DAEMON_PROCESS: running\n"
    else
        if should_alert "daemon_down"; then
            alert_red "🔴 Node Down" "$DAEMON_PROCESS is NOT running! Restart it (check launchctl if you run it as a LaunchAgent)."
        fi
        DETAILS+="🔴 $DAEMON_PROCESS: NOT RUNNING\n"
        ISSUES=$((ISSUES + 1))
...
# macOS has no systemd. If LAUNCHD_LABEL is set, the LaunchAgent is
# checked via launchctl; otherwise the process check stands in. Oracle
# process state comes from listoracle, same as Linux v1.5.
check_services() {
    if [ -n "$LAUNCHD_LABEL" ]; then
        ...
        if pgrep -x "$DAEMON_PROCESS" > /dev/null 2>&1; then
            DETAILS+="✅ $DAEMON_PROCESS process: running\n"
        else
            DETAILS+="🔴 $DAEMON_PROCESS process: not running\n"
            ISSUES=$((ISSUES + 1))
        fi
    fi
...
# version string incl. release candidate and git hash). Tries digibyted
# next to the configured digibyte-cli first, then on PATH. Falls back to
# the short getnetworkinfo subversion over RPC if the daemon binary can't
# be invoked (e.g. it lives inside the DigiByte-Qt app bundle).
check_version() {
    local version=""
    local cli_bin cli_dir
    cli_bin="${CLI%% *}"            # first token of CLI (path or bare name)
    if [[ "$cli_bin" == */* ]]; then
        cli_dir="${cli_bin%/*}"
        version=$("$cli_dir/digibyted" --version 2>/dev/null | head -1)
    fi
    if [ -z "$version" ]; then
        version=$("$DAEMON_PROCESS" --version 2>/dev/null | head -1)
    fi
    if [ -z "$version" ]; then
        version=$($CLI getnetworkinfo 2>/dev/null | jq -r '.subversion // empty' 2>/dev/null | tr -d '/')
    fi
    if [ -n "$version" ]; then
        DETAILS+="ℹ️ $version\n"
    fi
...
case "${1:-}" in
    ...
    --watch)
        ...
        WATCH_INTERVAL="${2:-60}"
        case "$WATCH_INTERVAL" in (*[!0-9]*|'') WATCH_INTERVAL=60 ;; esac
        DRY_RUN=true
        while true; do
            clear 2>/dev/null
            echo "🔭 Oracle Monitor — watch mode (refreshes every ${WATCH_INTERVAL}s, Ctrl+C to exit)"
            ISSUES=0
            WARNINGS=0
            DETAILS=""
            send_summary
