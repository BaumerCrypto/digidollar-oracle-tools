...
|---|---|---|---|
| Linux | [`oracle-monitor.sh`](oracle-monitor.sh) | [`config.template`](config.template) | 2.6.3 |
| Windows 10/11 | [`oracle-monitor.ps1`](oracle-monitor.ps1) | [`config.template.ps1`](config.template.ps1) | 2.6.3-win.1 |
| macOS | [`oracle-monitor-macos.sh`](oracle-monitor-macos.sh) | [`config-macos.template`](config-macos.template) | 2.6.3-macos.1 |
...
- **Footer** stamps the monitor version and your oracle identity on every card (e.g. `Oracle Monitor v2.6.3 — digibyte-maxi (ID 17)`). When an update is available, the footer gains a second line: `⬆️ v2.6.x available — https://github.com/BaumerCrypto/digidollar-oracle-tools` (v2.6.0)
...
```
   Set your Discord webhook URL, oracle ID, and oracle name. For mainnet, change `CLI="digibyte-cli"`. To enable email, set `EMAIL_ENABLED=true` and fill in the SMTP block. The template's inline docs cover Gmail App Password and Outlook; for the Brevo relay setup, see [CROSS_PLATFORM_SETUP.md](CROSS_PLATFORM_SETUP.md).
