...
| [oracle-monitor.ps1](oracle-monitor.ps1) | Windows PowerShell port v2.6.2-win.1 — full logic parity with Linux v2.6.2, including the v2.6.0 email + auto update-check features, the v2.6.1 cosmetic spacing fix, and the v2.6.2 version-line cleanup + UTC email timestamp + pressure-gated swap alert. PS 5.1 and PS 7 compatible, zero external dependencies (native JSON parsing, .NET's built-in `System.Net.Mail.SmtpClient` for email). Includes watch mode (`-Watch`), `-Config` for dual-instance monitoring, and `-TestEmail` for SMTP diagnostics. Ships UTF-8 with BOM. |
...
| [oracle-monitor-macos.sh](oracle-monitor-macos.sh) | macOS port v2.6.2-macos.1 — stock bash 3.2 compatible, jq is the only dependency (curl SMTP support ships with modern macOS). Full logic parity with Linux v2.6.2 (v2.6.0 email + update-check + v2.6.1 spacing fix + v2.6.2 version-line cleanup + UTC email timestamp + pressure-gated swap alert). Includes watch mode (`--watch`), `--config` for dual-instance monitoring, and `--test-email` for SMTP diagnostics. |
...
|---|---|---|---|
| Linux | [`oracle-monitor.sh`](oracle-monitor.sh) | [`config.template`](config.template) | 2.6.2 |
| Windows 10/11 | [`oracle-monitor.ps1`](oracle-monitor.ps1) | [`config.template.ps1`](config.template.ps1) | 2.6.2-win.1 |
| macOS | [`oracle-monitor-macos.sh`](oracle-monitor-macos.sh) | [`config-macos.template`](config-macos.template) | 2.6.2-macos.1 |
...
- **Footer** stamps the monitor version and your oracle identity on every card (e.g. `Oracle Monitor v2.6.2 — digibyte-maxi (ID 17)`). When an update is available, the footer gains a second line: `⬆️ v2.6.3 available — https://github.com/BaumerCrypto/digidollar-oracle-tools` (v2.6.0)
...
| oracle-monitor.sh | v2.6.2 |
| oracle-monitor.ps1 | v2.6.2-win.1 |
| oracle-monitor-macos.sh | v2.6.2-macos.1 |
